    ...
    def capture(
        ...
        def create_forward_fn(
            ...
            if not self.is_first_pp_rank:
                # Update for non-first PP ranks. Keep the raw tokens for models
                # that declare they need them on every stage (DeepSeek V4.1
                # routes image tokens off `input_ids` in every layer's gate);
                # capture must match what the real forward will be handed.

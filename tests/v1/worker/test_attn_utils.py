...
class FakeDeepseekV4FlashMLABackend:
    @staticmethod
    def get_kv_cache_shape(
        num_blocks: int,
        block_size: int,
...
def test_reshape_mla_kv_cache_uses_spec_cache_dtype_str():
    num_blocks = 3
    ...
    assert spec.real_page_size_bytes == 16 * 584
    ...
    attn_groups = [
        AttentionGroup(
            backend=FakeDeepseekV4FlashMLABackend,
            layer_names=["layer"],
            kv_cache_spec=spec,
            kv_cache_group_id=0,
        )
    ...
    assert kv_cache.shape == (num_blocks, 16, 584)
    assert kv_cache.stride(0) == spec.page_size_bytes
    assert kv_cache.stride(1) == 584
